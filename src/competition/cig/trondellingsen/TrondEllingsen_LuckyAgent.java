...
public final class TrondEllingsen_LuckyAgent implements Agent {
	...
	private final static boolean enemyInRange(final Environment e,
			final Rectangle r) {
		for (int i = 0; i < e.getEnemiesFloatPos().length; i += 3) {
			if (r.contains(e.getEnemiesFloatPos()[i + 1]
					- e.getMarioFloatPos()[0],
	...
	public final boolean[] getAction(final Environment observation) {
		...
		final boolean dangerOfEnemy = enemyInRange(observation, new Rectangle(
				-13, -57, 105, 87));
		final boolean dangerOfEnemyAbove = enemyInRange(observation,
				new Rectangle(-28, 28, 58, 45));
		final boolean dangerOfGap = dangerOfGap(observation
				.getLevelSceneObservation());
		if ((observation.isMarioOnGround() || observation.mayMarioJump())
				&& !jumpType.equals(JumpType.NONE)) {
			setJump(JumpType.NONE, -1);
		} else if (observation.mayMarioJump()) {
			final int wallHeight = getWallHeight(observation
					.getLevelSceneObservation());
			if (dangerOfGap && marioSpeed > 0) {
				setJump(JumpType.GAP, marioSpeed < 6 ? (int) (9 - marioSpeed)
						: 1);
			} else if (marioSpeed <= 1 && !dangerOfEnemyAbove && wallHeight > 0) {
				setJump(JumpType.WALL, wallHeight >= 4 ? wallHeight + 3
						: wallHeight);
		...
		final boolean isFalling = prevPos[1] < observation.getMarioFloatPos()[1]
				&& jumpType.equals(JumpType.NONE);
		action[Mario.KEY_LEFT] = isFalling
				&& ((dangerOfEnemy && dangerOfEnemyAbove) || dangerOfGap);
		action[Mario.KEY_RIGHT] = !isFalling
				&& !(dangerOfEnemyAbove && jumpType == JumpType.WALL);
		action[Mario.KEY_JUMP] = !jumpType.equals(JumpType.NONE)
				&& jumpCount < jumpSize;
		action[Mario.KEY_SPEED] = !(jumpType.equals(JumpType.ENEMY)
